...
**Note:** this is the "per-architecture" repository for the `s390x` builds of [the `caddy` official image](https://hub.docker.com/_/caddy) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![s390x/caddy build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/s390x/job/caddy.svg?label=s390x/caddy%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/s390x/job/caddy/)
...
Most users deploying production sites will not want to rely on mounting files into a container, but will instead base their own images on `s390x/caddy`:
...
Note the second `FROM` instruction - this produces a much smaller image by simply overlaying the newly-built binary on top of the the regular `s390x/caddy` image.
...
The `s390x/caddy` images come in many flavors, each designed for a specific use case.
